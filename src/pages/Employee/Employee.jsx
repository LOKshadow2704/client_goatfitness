  ...
  return (
    <div className={style["container"]}>
      <div className={style["menu"]}>
        <div className={style.infoEmployee}>
          ...
          <span>
            <FontAwesomeIcon
              icon={faCircle}
              style={{ color: user.TrangThai === "0" ? "green" : "gray" }}
            />
            {user.TrangThai === "0" ? "Online" : "Offline"}
          </span>
        </div>
        <ul>
          <li onClick={() => setCurrentPage("Dashboard")}>
            {" "}
            <FontAwesomeIcon icon={faDumbbell} /> &nbsp; Dashboard
          </li>
          <li onClick={() => setCurrentPage("Quản lý sản phẩm")}>
            <FontAwesomeIcon icon={faList} /> &nbsp; Quản lý sản phẩm
          </li>
          <li onClick={() => setCurrentPage("Quản lý danh mục sản phẩm")}>
            <CategoryIcon /> &nbsp; Quản lý loại sản phẩm
          </li>
          <li onClick={() => setCurrentPage("Quản lý gói tập")}>
            <FontAwesomeIcon icon={faFolderOpen} /> &nbsp; Quản lý gói tập
          </li>
          <li onClick={() => setCurrentPage("Gói tập của khách hàng")}>
            <FontAwesomeIcon icon={faAddressCard} /> &nbsp; Gói tập của khách
            hàng
          </li>
          {/* <li onClick={()=>setCurrentPage('Quản lý lịch dạy HLV')}><FontAwesomeIcon icon={faAddressCard}  /> &nbsp; Lịch dạy của HLV</li> */}
          <li onClick={() => setCurrentPage("Đơn hàng")}>
            <FontAwesomeIcon icon={faClipboard} /> &nbsp; Đơn hàng
          </li>
          <li>
            <FontAwesomeIcon icon={faUserGear} /> &nbsp; Tài khoản của bạn
            <ul className={style.dropdown}>
              <li onClick={() => setCurrentPage("Tài khoản của bạn")}>
      ...
      <div className={style["content"]}>
        <h1>
          {currentPage} {error || success || warning ? <Announcement /> : null}
        </h1>
        {currentPage === "Dashboard" && <Dashboard />}
        {currentPage === "Quản lý sản phẩm" && <ManageProduct />}
        {currentPage === "Quản lý danh mục sản phẩm" && (
        ...
        {currentPage === "Tài khoản của bạn" && (
          <div className={style.AccountInfo}>
            <AccountSetting changeForm={true} />
          </div>
        )}
      </div>
    </div>
