...
function Header() {
  ...
  <div className={style["user"]}>
    ...
    <p>{user.HoTen}</p>
    <ul className={style["dropdown-content"]}>
      {/* Các mục hiển thị nếu IDHLV === null */}
      {user.IDHLV === null && (
        <>
          <li>
            <Link to="/account-setting">Thông tin tài khoản</Link>
      ...
      {/* Các mục hiển thị nếu IDHLV có giá trị */}
      {user.IDHLV !== null && (
        <>
          <li>
            <Link to="/account-setting">Thông tin tài khoản</Link>
      ...
      {/* Mục đăng xuất hiển thị trong cả hai trường hợp */}
      <li onClick={Logout}>Đăng xuất</li>
    </ul>
  </div>
)}
      ...
      {showRegistPTModal && <ManagePTRegist setShowModal={setShowRegistPTModal} />}
